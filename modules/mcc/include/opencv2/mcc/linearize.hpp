    ...
    namespace ccm {
        ...
        public:
            ...
            cv::Mat linearize(cv::Mat inp) {
                ...
                merge(channel, res);// todo merge合并
                return res;
            };
        };

        /* get linearization method */
        Linear* get_linear(double gamma, int deg, cv::Mat src, Color dst, cv::Mat mask, RGB_Base_ cs, LINEAR_TYPE lineat_type) // TODO 智能指针
        {
            Linear* p = new Linear(); // todo 初始化为NULL或者nullptr,智能指针
            switch (lineat_type)
            {
