...
namespace cv {
	namespace ccm {
		...
		public:
			...
			cv::Mat grays;
			cv::Mat colored;
			std::map<ColorSpace, Color*> _history;  // todo 取消指针&引用&智能指针
			...
			Color to(const ColorSpace& other, CAM method = BRADFORD, bool save = true) {
				...
				Color* color = new Color(ops.run(colors), other);// todo 改成智能指针
				if (save) {
			...
			cv::Mat toGray(IO io, CAM method = BRADFORD, bool save = true) {
				XYZ xyz(io); // todo 在colorspace里创建map保存xyz和lab，不重复创建colorspace
				return channel(this->to(xyz, method, save).colors, 1);
			}

			cv::Mat toLuminant(IO io, CAM method = BRADFORD, bool save = true) {
				Lab lab(io);// todo 在colorspace里创建map保存xyz和lab，不重复创建colorspace
		...
		const cv::Mat ColorChecker2005_LAB_D50_2 = (cv::Mat_<Vec3d>(24, 1) <<
		...
		const cv::Mat ColorChecker2005_LAB_D65_2 = (cv::Mat_<Vec3d>(24, 1) <<
